...
class TiffBuffer
{
	public:
		...
		bool isEmpty() const noexcept {return (w * h * channels) == 0;}
...
std::vector<std::uint8_t> segmentation::readTiffImage(const char * filePath, 
	...
	out_data.reserve(buff.getW() * buff.getH() * buff.getChannels());
	...
	for(std::size_t y = yMin; y < yMax; ++y){
		for(std::size_t x = xMin; x < xMax; ++x){
			for(typeUndEnum channel = 0; channel < buff.getChannels(); ++channel){
				out_data.push_back(buff.get(x, y,  static_cast<TiffBuffer::TiffChannel>(channel))); 
			} 							
		}
	} 
