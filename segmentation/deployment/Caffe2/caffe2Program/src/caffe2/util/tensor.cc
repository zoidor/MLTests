...
TensorCPU TensorUtil::ScaleImageTensor(int width, int height) {
  ...
  at::IntList dims({count, dim_c, height, width});
  return caffe2::TensorCPUFromValues(dims, at::ArrayRef<decltype(output)::value_type>{output});
}
...
void image_to_tensor(TensorCPU &tensor, cv::Mat &image, float mean = 128) {
  ...
  at::IntList dims({1, 3, image.rows, image.cols});
...
void read_image_tensor(TensorCPU &tensor,
  ...
  // create tensor
  at::IntList dims({static_cast<int>(indices.size()), 3, height, width});
  auto t = caffe2::TensorCPUFromValues(dims, at::ArrayRef<T>{data});
  tensor.ResizeLike(t);
  tensor.ShareData(t);
}
